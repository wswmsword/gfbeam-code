(function() {
   var horizontalElem = document.getElementById('project-list'),
      ...
      uriList = '/proxy/explore/listProjectsByGeo',
      ...
      constLychees = {
         ...
         coord: decodeURIComponent(location.pathname.slice(location.pathname.lastIndexOf('/') + 1)),
      },
      copyLemons = {
         server: {
            membership: {
   ...
   function generateEarthPlate(list) { // 大地石板
      ...
      for (i = 0; i < length; ++ i){
         ...
         list_html += '<div class="project-holder">'
                        ...
                        + '</span>'
                        + '<div class="scrollY">'
                           + '<a class="button detail-button" href="' + '/time/corewrite/' + list[i].id + '">记录光阴</a>'
                           + '<span class="button detail-button cancel">返回</span>'
                           + '<span class="button detail-button preview">预览</span>'
   ...
   var functionGrapes = { // 🍇
      eventhandler: {
         more2ClickHandler: function(e) {
            ...
            util.serve('get', util.url(uriList, { i: pager.incr(), c: constLychees.coord }), functionGrapes.servercallback.postMore2AutoScroll);
         },
   ...
   function initData() {
      ...
      //  初始化展示首页项目
      util.serve('get', util.url(uriList, { i: pager.incr(), c: constLychees.coord }), functionGrapes.servercallback.getlistInit);
